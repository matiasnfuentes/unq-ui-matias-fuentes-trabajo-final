...
const getRandom = (quantity) =>
  [...items].sort(() => 0.5 - Math.random()).slice(0, quantity);

export const getRandomRows = (itemsNeeded, boardSize, rowsCount) => {
  const items = getRandom(itemsNeeded);
  let shuffled = [...items, ...items]
    .sort(() => 0.5 - Math.random())
    .map((i, index) => ({ ...i, uniqueId: index }));
  return [...Array(rowsCount).keys()].map((r) => shuffled.splice(0, boardSize));
};
